...
Paste your full GAS source between the fences below. Inside a code block, markdown will not touch any characters — no underscores, asterisks, or backslashes will be altered.
  ...
  return withLock_(() => {
    try {
      ...
      if (action === "setchecklist") {
        const result = setChecklist_(p);
        return jsonOut_({ ok: true, ...result });
      }
      return jsonOut_({
        ok: false,
        error: "Unknown action. Use create|update|delete|saveWeekNote|setChecklist.",
      });
    } catch (err) {
...
function getChecklist_(p) {
  ...
  const rows = readAllAsObjects_(sheet, HEADERS.Checklist).filter(
    (r) => normalizeCellDateToYMD_(r.date) === date,
  );
  return { ok: true, rows };
}

function setChecklist_(p) {
  ...
  try {
    ...
    switch (fn) {
      ...
      case "updateTripItineraryPdf":
        ...
        break;
      case "getChecklist":
        data = getChecklist_(p);
        break;
      case "listLog": {
        const ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
        const logSheet = ss.getSheetByName(CONFIG.SHEET_LOG);
        const allRows = logSheet ? readAllAsObjects_(logSheet, HEADERS.Log) : [];
      ...
      default:
        data = {
          ok: false,
          error:
            "Unknown fn. Use weekData|listTrips|getTrip|listDrivers|listBuses|getBusAssignments|listBusAssignmentsForRange|listLog|logError.",
        };
    }
 ...
 * TRIPS CRUD
 * ============================= */
function getBusAssignmentRow_(busSheet, tripKey) {
  const idx = findRowIndexByValue_(busSheet, "tripKey", tripKey);
  if (idx < 0) return {};
  return getRowObject_(busSheet, HEADERS.BusAssignments, idx);
}
...
function appendLog_(ss, entry) {
  ...
  if (!sheet) return;
  appendRowByHeaders_(sheet, HEADERS.Log, {
    timestamp: new Date(),
...
function createTrip_(p) {
  ...
  appendLog_(ss, { tripKey, tripId, action: "trip_added" });
...
function updateTrip_(p) {
  ...
  const depD_forId =
    safeDateYMD_(p.departureDate) || coerceToDate_(existing.departureDate) || new Date();
  ...
  if (incomingPdfUrl && !existingPdfUrl) {
    appendLog_(ss, { tripKey, tripId, action: "itinerary_uploaded", newValue: incomingPdfUrl });
  }

  const existingBusRow = getBusAssignmentRow_(busSheet, tripKey);
  const DRIVER_FIELDS = [
    "driver1",
    "driver2",
    "driver3",
...
function deleteTrip_(p) {
  ...
  const deleteIdx = findRowIndexByValue_(tripsSheet, "tripKey", tripKey);
  const deletedRow = deleteIdx >= 0 ? getRowObject_(tripsSheet, HEADERS.Trips, deleteIdx) : {};
  ...
  if (removed) {
    appendLog_(ss, { tripKey, tripId: deletedTripId, action: "trip_deleted" });
  }
...
function saveWeekNote_(p) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]).trim() === "global") {
      foundRow = i + 1;
      break;
    }
  }
  ...
  PropertiesService.getScriptProperties().setProperty("GLOBAL_NOTE", noteText);
  invalidateWeekCache_();
...
function uploadItineraryPdf_(e) {
  ...
  if (rowIndex > 0) {
    const headerRow = tripsSheet
      .getRange(1, 1, 1, tripsSheet.getLastColumn())
      .getValues()[0]
      .map(String);
...
function ensureAllSheets_() {
  ...
  const unavail =
    ss.getSheetByName(CONFIG.SHEET_UNAVAILABILITY) || ss.insertSheet(CONFIG.SHEET_UNAVAILABILITY);
  const checklist =
    ss.getSheetByName(CONFIG.SHEET_CHECKLIST) || ss.insertSheet(CONFIG.SHEET_CHECKLIST);
  ...
  ensureHeaders_(checklist, HEADERS.Checklist);
  ensureHeaders_(log, HEADERS.Log);
}
...
function MIGRATE_normalizeTripsDatesAndTimes() {
  ...
  if (idxDepDate < 0 || idxArrDate < 0 || idxDepTime < 0 || idxArrTime < 0) {
    throw new Error(
      "Missing one of the required columns: departureDate, arrivalDate, departureTime, arrivalTime",
    );
  }
  let changed = 0;
  for (let r = 1; r < values.length; r++) {
