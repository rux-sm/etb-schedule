        ...
        for (let assignIdx = 0; assignIdx < sortedAssigns.length; assignIdx++) {
            ...
            if (bar._bInv) {
                const inv = String(t.invoiceStatus || "")
                    ...
                    .toLowerCase();
                const showInv =
                    inv === "invoiced" || inv === "deposit received" || inv === "paid in full";
...
async function openTripDetailsModal(tripKey) {
    try {
        ...
        t = tripResp.trip || {};
        assigns =
            assignResp?.ok && Array.isArray(assignResp.assignments) ? assignResp.assignments : [];
...
async function openTripForEdit(tripKey) {
    ...
    try {
        ...
        [
            ...
            "driverStatus",
            "invoiceStatus",
        ].forEach((id) => updateStatusSelect($(id)));
        updateInvoiceNumberVisibility();
...
function buildPrintScheduleTwoPages() {
    ...
    function makeTableForRows(startIdx, endIdx) {
        ...
        if (headerClone) {
            ...
            headerClone
                .querySelectorAll(
                    ".nav-controls, .date-input-overlay, input.weekpicker, .weekpicker-trigger-wrap",
                )
    ...
    printRoot
        ...
        .forEach((t) => repositionBarsForPrint(t, colMetrics));
...
async function loadDriversAndBuses(forceRefresh = false) {
    ...
    state.busesList = state.busesList
        .map((b) => ({
            ...
            busName:
                b.busName && String(b.busName).trim() ? String(b.busName).trim() : `Bus ${b.busId}`,
        }))
        .filter((b) => b.busId);
...
function wireEvents() {
    ...
    dom.driverWeekBody.addEventListener("mousedown", (e) => {
        ...
        state.dragSelection.mode = td.classList.contains("driver-cell-unavailable")
            ? "remove"
            : "add";
    ...
    window.addEventListener("mouseup", async () => {
        ...
        toastShow(
            mode === "add" ? "Marking as unavailable..." : "Marking as available...",
            "loading",
        );
    ...
    dom.tripForm.addEventListener("submit", (e) => {
        ...
        ["req56Pass", "reqSleeper", "reqLift", "reqRelief", "reqCoDriver", "reqHotel"].forEach(
            (id) => {
                const btn = $(id);
