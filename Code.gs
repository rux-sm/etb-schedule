...
function updateTrip_(p) {
  ...
  // ============================
  // OPTION A (PATCH):
  // tripId is IMMUTABLE after create.
  // Ignore any incoming p.tripId and always keep existing.tripId.
  // If missing (legacy rows), generate it ONCE and store it.
  // ============================
  ...
  // Normalize dates (store as YYYY-MM-DD)
  ...
  // Make sure tripKey in row stays correct
  p.tripKey = tripKey;
  // Hard-ignore any incoming tripId so it can’t overwrite by mistake
...
function mapTripFromParams_(p, base) {
  ...
  return {
    ...
    tripColor: String(p.tripColor || "").trim(), // <--- NEW
    // ✅ NEW MEANINGS
    notes: incomingNotes, // short “Notes” (was barNotes)
